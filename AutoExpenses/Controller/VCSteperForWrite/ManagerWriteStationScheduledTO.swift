...
class ManagerWriteStationScheduledTO {
    ...
    private var arrayNameTO: [String]?
    ...
    func reloadJSON() {
      ...
      if let arrayTypesTO = ParsingJSON(nameFile: "Toyota LC 200 VD").getDataFromJson(key: "kilm") as? [Any] {
        ...
        for item in arrayTypesTO {
            ...
            self.arrayNameTO?.append(model.dataTO!.nameTO!)
            self.arrayModelsWriteTO.append(model)
        }
        
        self.selectedCurrentModel = self.arrayModelsWriteTO![self.currentIndexTO]
            setIndex(index: self.currentIndexTO)
        }
    }
    
    func setDateVisit(dateVisit: Date) {
        self.selectedCurrentModel!.dateVisit = dateVisit
    }
    
    func setIndex(index: Int) {
        self.currentIndexTO = index
        selectedCurrentModel?.dataTO = arrayModelsWriteTO?[index].dataTO
    }
